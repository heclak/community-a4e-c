...
function update()	
    ...
    if external_power_connected then
        -- This is the original code from pre 1.02. Movement detection code is incorrect for carrier ops as the carrier will move the aircraft around the map. getSelfCoordinates should not be used.
        -- DCS will disconnect external power if aircraft movement is detected. There is internal code for detecting aircraft movement. This internal logic accounts for the carrier movement so it is ideal.
        -- This code is left as a note of what did not work.
	...
	set_aircraft_draw_argument_value(HUFFER_ANIM_ARG, HUFFER_STATE)
	-- lastrpm = rpm
